...
class NeuralShader(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if fourier_features == 'gfft':
            ...
            self.diffuse = CONV(mapping_size, hidden_features_size, kernel_size, stride, [hidden_features_size] * hidden_features_layers, activation, None).to(device)
            ...
            self.diffuse = CONV(channels, hidden_features_size, kernel_size, stride, [hidden_features_size] * hidden_features_layers, activation, None).to(device)
            self.specular = CONV(hidden_features_size+3+3, 3, kernel_size, stride, [hidden_features_size // 2], activation, last_activation).to(device)
        elif fourier_features == 'none':
            self.diffuse = CONV(3, hidden_features_size, kernel_size, stride, [hidden_features_size] * hidden_features_layers, activation, None).to(device)
            self.specular = CONV(hidden_features_size+3+3, 3, kernel_size, stride, [hidden_features_size // 2], activation, last_activation).to(device)
