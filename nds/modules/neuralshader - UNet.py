...
from nds.modules.unet_specular import UNET_specular
from nds.modules.unet_diffuse import UNET_diffuse
...
class NeuralShader(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if fourier_features == 'gfft':
            ...
            self.diffuse = UNET_diffuse(mapping_size, hidden_features_size, kernel_size, stride, [hidden_features_size, hidden_features_size * 2, hidden_features_size * 4], activation, None).to(device)
            self.specular = UNET_specular(hidden_features_size+3+3, 3, kernel_size, stride, [hidden_features_size, hidden_features_size * 2], activation, last_activation).to(device)
        elif fourier_features == 'positional':
            self.fourier_feature_transform, channels = get_embedder(fft_scale)
            self.diffuse = UNET_diffuse(channels, hidden_features_size, kernel_size, stride, [hidden_features_size, hidden_features_size * 2, hidden_features_size * 4], activation, None).to(device)
            self.specular = UNET_specular(hidden_features_size+3+3, 3, kernel_size, stride, [hidden_features_size, hidden_features_size * 2], activation, last_activation).to(device)
        elif fourier_features == 'none':
            self.diffuse = UNET_diffuse(3, hidden_features_size, kernel_size, stride, [hidden_features_size, hidden_features_size * 2, hidden_features_size * 4], activation, None).to(device)
            self.specular = UNET_specular(hidden_features_size+3+3, 3, kernel_size, stride, [hidden_features_size, hidden_features_size * 2], activation, last_activation).to(device)
    ...
    def forward(self, position, normal, view_dir):
        ...
        if self.fourier_feature_transform is not None:
            h = self.diffuse(self.fourier_feature_transform(position).permute(2, 0, 1).unsqueeze(0))
            h = torch.cat([h, normal.permute(2, 0, 1).unsqueeze(0), view_dir.permute(2, 0, 1).unsqueeze(0)], dim=1)
        else:
            h = self.diffuse(position.permute(2, 0, 1).unsqueeze(0))
            h = torch.cat([h, normal.permute(2, 0, 1).unsqueeze(0), view_dir.permute(2, 0, 1).unsqueeze(0)], dim=1)
