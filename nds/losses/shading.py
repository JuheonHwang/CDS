...
def shading_loss(views: List[View], gbuffers: List[Dict[str, torch.Tensor]], shader, loss_function=torch.nn.L1Loss(), shading_percentage=1):
    ...
    for view, gbuffer in zip(views, gbuffers):
        ...
        view_direction[mask] = 0
        
        shade = shader(position, normal, view_direction)
        image_loss = loss_function(shade, target)
        ssim_loss = 1 - ssim(torch.permute(shade.unsqueeze(0), [0, 3, 1, 2]), torch.permute(target.unsqueeze(0), [0, 3, 1, 2]), data_range=1.0, size_average=True)

        loss += (image_loss + 0.75 * ssim_loss)
