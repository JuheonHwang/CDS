...
def generate_mesh(generator_name: str, views: List[View], aabb: AABB, device: torch.device = 'cpu'):
    ...
    mesh_generators = {
        ...
        'vh128': (lambda: compute_visual_hull(views, aabb, grid_size=128, device=device)),
        'vh256': (lambda: compute_visual_hull(views, aabb, grid_size=256, device=device)),
        'vh512': (lambda: compute_visual_hull(views, aabb, grid_size=512, device=device)),
